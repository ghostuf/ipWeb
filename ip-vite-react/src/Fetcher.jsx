...
export default function Fetcher() {
  ...
  const buttonhandler = () => {
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        divElem.innerHTML = JSON.stringify(
          `Country => ${data.country}<br>Region => ${data.regionName}<br> City => ${data.city}<br> isp => ${data.isp}`
        );
      })
      .catch((e) => {
        console.log(`not able to fetch data! error: ${e}`);
      });
  };
  ...
  return (
    <div className="input-terminal-container">
      ...
      <div className="output-terminal"></div>
    </div>
  );
}
